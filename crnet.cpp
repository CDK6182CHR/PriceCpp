...
std::unique_ptr<CRNet> CRNet::instance;

CRNet::CRNet():
    Graph<QString>(5000),currentSource(-1),currentTarget(-1),currentSsspResult(INF)
{
    read();
    qDebug()<<"CRNet vertex count"<<size<<Qt::endl;
}
...
CRNet *CRNet::getInstance()
{
    if (instance)
        ...
        instance=std::unique_ptr<CRNet>(new CRNet);
        return instance.get();
    }
}
...
int CRNet::mileBetween(const QString &from, const QString &to, QStringList &path)
{
    int v=getVertexPos(from),
        w=getVertexPos(to);
    if(v==-1||w==-1){
        qDebug()<<"whether source or target not existed in graph: "<<from<<" "
            <<to<<Qt::endl;
        return -1;
    }
    currentSource=v;
...
int CRNet::read()
{
    ...
    in.setCodec("UTF-8");
    if(!file.isOpen()){
        qDebug()<<"Fetal: Cannot open file "<<fileName<<Qt::endl;
        return 1;
    }
    while(!in.atEnd()){
        ...
        if(list.length()!=4){
            qDebug()<<"Invalid mile file data: "<<line<<Qt::endl;
            continue;
        }
        lineName=list[0];
        ...
        mile=list[3].toInt();
        if(mile<=0){
            qDebug()<<"Invalid negative weight! "<<line<<Qt::endl;
            continue;
        }
